...
void solve(double meal_cost, int tip_percent, int tax_percent) {
  ...
  tmpcost = meal_cost * (100.0 + (double)tip_percent + (double)tax_percent) / 100.0;
  totalcost = (int)(tmpcost + 0.5);
...
int main()
{
    ...
    if (meal_cost_endptr == meal_cost_str ||
       *meal_cost_endptr != '\0') {
      exit(EXIT_FAILURE);
    }
    ...
    if (tip_percent_endptr == tip_percent_str ||
       *tip_percent_endptr != '\0') {
      exit(EXIT_FAILURE);
    }
    ...
    if (tax_percent_endptr == tax_percent_str ||
       *tax_percent_endptr != '\0') {
      exit(EXIT_FAILURE);
    }
